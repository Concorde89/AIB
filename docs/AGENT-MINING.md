...
```bash
curl -s "https://oracle.x402endpoints.online/v1/addresses" | jq -c '{addresses}' > ~/.aib/oracle_cache.json
```
...
1. **Oracle cache missing**: Create `~/.aib/oracle_cache.json` and restart node
